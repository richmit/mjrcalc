...
;; @Copyright Copyright 1998,2008,2011,2013 by Mitch Richling.  All rights reserved.
;; @brief     Character (ASCII & EBCIDIC) tools.@EOL
...
If a character code maps to a non-printable character, then the returned string is a typical representation of the value -- DEL for
the delete character for example (ASCII 127). The return is nil if the integer can not be converted.
...
Note that CHAR may be a multiple character string that represents a non-printable character like DEL (ASCII 127). The return is nil
if the integer can not be converted.
...
Arguments:
  ...
  :base is used to specify the base of the numeric output.  It may be one of :d (decimal), :h (hex), or :o (oct)
  It may also be an integer (16, 10, or 8).        

  :rows is used to specify the number of rows used for the print out.  The default is 16.  Other values that yield attractive
  print outs are 8, 32, 64, 128, and 256.

  :char-set is used to specify the character set.  It may be one of :cs-ascii or :cs-ebcdic.  In the case of :cs-ebcdic, only the
  characters that have corresponding ASCII equivalents are printed."
