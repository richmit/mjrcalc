  ...
  (let ((fun (mjr_mxp_string-or-func-to-lambda fun "x")))
    (multiple-value-bind (start end step len)
        (mjr_util_get-kwarg-vals '(:start :end :step :len) 
                                 (mjr_vvec_normalize-vvt-aseq (list :start start :end end :step step :len len)))
    ...
    (flet ((simpson (a b fa fm fb) (* (/ (- b a) 6) (+ fa (* 4 fm) fb))))
      (let ((ect   0)
        ...
        (let ((ilist (loop for x0 = nil then x1
                           for x1 in (mjr_vvec_gen-0sim 'list (list :points points :start start :end end :step step :len len :map-fun seq-next))
                           for f0 = nil then f1
...
be used and a warning will be printed unless :SUPPRESS-WARNINGS is non-NIL."
  (let ((fun (mjr_mxp_string-or-func-to-lambda fun "x")))
    ...
    (let* ((points (mjr_vvec_gen-0sim 'vector vvec))
           (len    (length points)))
