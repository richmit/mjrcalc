...
This library implements completely deterministic, rational approximations for some irrational functions.  By 'deterministic' we mean
that a finite sequence of rational arithmetic operations are used to compute the approximation.  The results will be consistent
across platforms so long as arbitrary precision integer and rational arithmetic are correctly implemented in the LISP being used.

WARNING: Take care to note the ranges upon which the functions are defined.  Example: tan-4 is good on [-1,1], while tan-4 is valid
         for any value not near a singularity.

Some Use Cases:

  * Employ standard test cases involving irrational functions for unit tests of numerical algorithms.  Having a known, rational
    value that should result from a test case is much less difficult to test and better evidence of proper function.
...
Uses a low degree polynomial hand fitted to sin and its derivative over 0..2*pi.  pi=355/113
Most accurate near 0, pi, and 2*pi.  Least accurate extremes near about 0.5 & 5.7."
  (if (< x 0)
      (- (mjr_rirrf_sin-4 (- x)))
...
Uses a single polynomial hand fitted to sin and its derivative.  pi=355/113.  Zeros at -355/113, 0, and 355/113.  Value of -1 at
-355/226, and +1 at 355/226.  Derivative is zero at -355/226 and 355/226.  Derivative is -1 at -355/113 and 355/113, and it is 1 at
0.  The approximation is most accurate near 0, pi, and 2*pi.  The approximation is an odd function."
...
(defun mjr_rirrf_sin-2 (x)
  "Rational approximation to sin function.  
Uses a low degree polynomial hand fitted to sin and its derivative over 0..2*pi.  pi=22/7.
Most accurate near 0, pi, and 2*pi."
  (if (< x 0)
      (- (mjr_rirrf_sin-2 (- x)))
...
Uses a low degree polynomial hand fitted to exp and its derivative.
Most accurate near 0 and 1."
  (if (or (< x 0) (> x 1))
...
Uses a low degree polynomial hand fitted to exp and its derivative.
Most accurate near 0, 1/2, and 1."
  (if (or (< x 0) (> x 1))
...
Uses a slightly modified Chebyshev approximation with rationalized polynomial coefficients.  This is an odd function, zero at zero,
with derivative of 1 at zero."
  (if (or (< x -1) (> x 1))
...
Uses a low degree polynomial hand fitted to sqrt and its derivative.
Most accurate near 1/2, 1, and 3/2 -- exact at 1"
  (if (or (< x 1/2) (> x 3/2))
