...
;; @Keywords  lisp interactive permutation group math library
;; @Std       Common Lisp
;;
;;            
;;            
...
This library performs some basic permutation group computations that are of use for the other computations.  In keeping with the
'utility' focus, most of the permutations are zero-indexed; however, a few of the functions allow this to be changed (the
index-base argument) to support more interactive use.  That said, it is unlikely that this library will ever be extended into a
general permutation group computation tool -- use GAP for that. :)"
...
A permutation may be invalid for any number of reasons: 1) It is not a vector, 2) It contains non-integers, 3) It contains out
of range integers, 4) It contains duplicate integers."
  (if (vectorp perm)
      (let* ((len     (length perm))
