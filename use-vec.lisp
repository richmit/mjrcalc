...
;; @Keywords  lisp interactive vectors matrix
;; @Std       Common Lisp
;;
...
This function is careful to avoid floating point overflow when possible, but pays a price in performance.  Underflow is still
a problem. Also note that it still gets wrong answers for some combinations of inputs ; however, it is better than the naive
implementation."
  (if (numberp vec)
      (abs vec)
      (let ((len (length vec)))
...
If a VECTOR is not a vector, then it will be printed out with ~S~& regardless of what was given for FMT-STR..
Returns VECTOR so that this function may be inserted inside nested function calls."
  (if (vectorp vector)
...
(defun mjr_vec_< (a b &key (order :lex))
  "Vector orderings
All of the orderings are suitable for use as monomial orderings for commutative algebra computations -- the names used are
typical"
