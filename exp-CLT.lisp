...
;; @Copyright Copyright 1993,1995,1996,1997,2003,2008,2009,2010,2012 by Mitch Richling.  All rights reserved.
;; @brief     Theoretical family of probability distributions of sums of an input PDF represented as a historgram.@EOL
;; @Keywords  theoretical family probability distributions sums pdf histogram
;; @Std       Common Lisp
;;
...
rv-dat is an array of discrete random variable values (non-negative integers). wt-dat is an array of weights i.e. the
probability, density, or count.  This function transforms the weights into a PDF by dividing the weights by a constant value so
that they will all sum to 1.  

A family of max-nsum theoretical probability distributions is returned, each as a column of the returned 2D array.  The i'th PDF
is the PDF resulting from summing i independent copies of the input PDF.

This function allows us to see the central limit theorem in action if we divide each derived random variable by i.  It can be a
real eye-opener to see just how quickly, or slowly, a particular distribution's mean sampling distribution approaches normality!

The algorithm for doing the PDF computations is iterative using the i'th PDF for the computation of the (i+1)'th PDF.  This
renders the PDFs very quickly when compared to a naive implementation.  Even with a good algorithm, this kind of computation is
best done in a fast language (C++, Fortran, or a fast LISP implementation like SCBL)."
...
;;----------------------------------------------------------------------------------------------------------------------------------
;; Mathematical application: Most lower level statistics texts will describe an application of the central limit theorem to the
;; sampling distribution from arbitrary populations.  The fundamental result is that the sampling distribution will approach
;; normality as the sample size grows to infinity.  Students are normally given advice like "a sample of 30 is sufficient".  The
;; data in exCLT-IN.csv describes a very non-normal PDF.  The following code will read in that distribution, and compute the
;; sampling distribution from sample sizes up to 30.  The R code in exCLT-AUX.R illustrates how to use the results.
...
;;----------------------------------------------------------------------------------------------------------------------------------
;; Interesting compute farm/grid/cloud application: If the random variable is serial job CPU utilization (an integer from 0 to 100)
;; and the weights are the number of wall clock seconds consumed by jobs with that job CPU utilization, then the output PDFs will
;; represent HOST utilization (when we divide by the number of CPUs on the host) for various job counts.
